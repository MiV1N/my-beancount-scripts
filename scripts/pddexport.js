...
(function () {
    ...
    function extractCSV() {
        ...
        let csv = `下单时间,订单号,店铺名称,商品名称,商品型号,商品价格,订单状态,实付价格\n`;
        ...
        orderItems.forEach(item => {
            try {
                ...
                const orderId = orderIdElement ? orderIdElement.innerText.replace('订单号：', '').trim() : "";
    
    
                // 处理字段中本身有逗号的问题
                const fields = [orderTime,orderId,shopName, productName, productModel, price, status, paid];
                const formattedFields = fields.map(field => field.includes(',') ? `"${field}"` : field);
